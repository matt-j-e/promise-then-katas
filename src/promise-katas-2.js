...
const food = () => fetch("food").then((object) => object.data);
...
const cat = () => fetch("cats").then(object => object.data.cats.sort());
...
const dog = () => fetch("dogs")
    .then(object => object.data.dogs
    .sort((a, b) => a.naughty > b.naughty ? -1 : 1)[0]);
...
const joke = () => {
    const params = [["jokes", "question"], ["jokes", undefined]];
    const content = {};
    const response = Promise.all(params.map(param => {
        fetch(param[0], param[1])
        .then(res => {
            if (res.joke) content.question = res.joke;
            else if (res.answer) content.answer = res.answer;
        });
    }));
    return content;
}
